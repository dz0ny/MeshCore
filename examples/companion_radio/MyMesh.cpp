  ...
  if (cmd_frame[0] == CMD_DEVICE_QEURY && len >= 2) { // sent when app establishes connection
    ...
    if (!getChannel(channel_idx, channel)) {
      ...
      if (txt_type == TXT_TYPE_PLAIN) {
        success = sendGroupMessage(msg_timestamp, channel.channel, _prefs.node_name, (const char *) payload, payload_len);
      } else {
        success = sendGroupData(msg_timestamp, channel.channel, txt_type, payload, payload_len);
      }
