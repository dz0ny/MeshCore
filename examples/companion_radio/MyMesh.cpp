...
int MyMesh::getRecentlyHeard(AdvertPath dest[], int max_num) {
  ...
  for (int i = 0; i < max_num; i++) {
    ...
    // Initialize GPS to 0 (no data)
    dest[i].gps_lat = 0;
    dest[i].gps_lon = 0;

    // Try to enrich with GPS data from contacts
    // Only search if we have a valid node (name not empty)
    if (dest[i].name[0] != 0) {
      ContactInfo contact;
      for (int j = 0; j < getNumContacts(); j++) {
        // Get contact by index using public API
        if (getContactByIdx(j, contact)) {
          // Match by first byte of public key (unique hash)
          if (contact.id.pub_key[0] == dest[i].pubkey_prefix[0]) {
            // Found matching contact, copy GPS data if available
            dest[i].gps_lat = contact.gps_lat;
            dest[i].gps_lon = contact.gps_lon;
...
void MyMesh::onMessageRecv(const ContactInfo &from, mesh::Packet *pkt, uint32_t sender_timestamp,
                           const char *text) {
#ifdef NRF52_PLATFORM
  nrf52::resetWatchdog();  // Reset watchdog on LoRa packet received
#endif
...
MyMesh::MyMesh(mesh::Radio &radio, mesh::RNG &rng, mesh::RTCClock &rtc, SimpleMeshTables &tables, DataStore& store, AbstractUITask* ui)
  ...
  _prefs.gps_loc_distance_threshold = 5;   // 5 meters
  _prefs.gps_loc_frequency = 3;            // 30 seconds (stored as /10)
  _prefs.gps_loc_guaranteed_interval = 1;  // 2 minutes
...
void MyMesh::handleCmdFrame(size_t len) {
  if (cmd_frame[0] == CMD_DEVICE_QEURY && len >= 2) { // sent when app establishes connection
    ...
    if (pkt) {
      if (len >= 2 && cmd_frame[1] == 0) { // optional param (1 or default = flood, 0 = zero hop)
        sendZeroHop(pkt);
      } else {
        sendFlood(pkt);
      }
...
void MyMesh::sendLocationAdvertisement(double lat, double lon) {
  ...
  if (pkt) {
    int pkt_size = pkt->getRawLength();
    sendFlood(pkt);
    MESH_DEBUG_PRINTLN("Location advert SENT successfully - size: %d bytes", pkt_size);
  } else {
