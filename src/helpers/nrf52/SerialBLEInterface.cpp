...
void SerialBLEInterface::onConnect(uint16_t connection_handle) {
  nrf52::resetWatchdog();  // Reset watchdog on BLE callback
...
void SerialBLEInterface::onSecured(uint16_t connection_handle) {
  ...
  if(instance){
    ...
    if(instance->_onConnectCallback){
      instance->_onConnectCallback();
    }
  }
}
...
size_t SerialBLEInterface::checkRecvFrame(uint8_t dest[]) {
  if (send_queue_len > 0   // first, check send queue
    ...
    nrf52::resetWatchdog();  // Reset watchdog on BLE data send
    ...
    if (len > 0) {
      ...
      nrf52::resetWatchdog();  // Reset watchdog on BLE data receive
